...
export const dashboardRouter = createRouter({
  ...
  getHrDashboard: authedQuery.query(async ({ ctx }) => {
    ...
    const nextMonthStartDate = workZoneWallTimeToUtc(
    ...
    const [
      ...
      // Attendance / clock-in hours only (exclude task timers — those are shown separately).
      sumDuration({
        ...tenant,
        taskId: null,
        clockIn: { $gte: thisMonthStartDate, $lt: nextMonthStartDate },
      }),
      sumDuration({
        ...tenant,
        taskId: { $ne: null },
      ...
      sumDuration({
        ...tenant,
        taskId: null,
        clockIn: { $gte: lastMonthStartDate, $lt: thisMonthStartDate },
      }),
      sumDuration({
        ...tenant,
        taskId: { $ne: null },
    ...
    // Working weekdays elapsed this month × headcount × required daily hours
    let weekdaysElapsed = 0;
    for (let day = 1; day <= nowParts.day; day++) {
      const key = workZoneDateKey(
        workZoneWallTimeToUtc(nowParts.year, nowParts.month, day, 12, 0, 0, 0),
      );
      if (isWeekdayDateKey(key)) weekdaysElapsed += 1;
    }
    const trackableHeadcount = activeUsers.filter(
      (u) => !isAdminOrManagement(u) && !isHrRoleOnly(u),
    ).length;
    const capacityHours =
      Math.max(trackableHeadcount, 1) * weekdaysElapsed * REQUIRED_DAILY_HOURS;
    const teamUtilizationPct =
      capacityHours > 0
        ? Math.min(100, Math.round((totalHoursLogged / capacityHours) * 100))
        : 0;

    let lastMonthWeekdays = 0;
    const lastMonthEndParts = workZoneDateParts(
      new Date(thisMonthStartDate.getTime() - 12 * 3600_000),
    );
    for (let day = 1; day <= lastMonthEndParts.day; day++) {
      const key = workZoneDateKey(
        workZoneWallTimeToUtc(
          lastMonthStartParts.year,
          lastMonthStartParts.month,
      ...
      if (isWeekdayDateKey(key)) lastMonthWeekdays += 1;
    }
    const lastCapacity =
      Math.max(trackableHeadcount, 1) * lastMonthWeekdays * REQUIRED_DAILY_HOURS;
